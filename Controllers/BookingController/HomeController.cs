...
namespace BookingForm.Controllers
{
    public class HomeController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> PassportInfo(Guid id)
        {
            var app = await _context.appoinment.FirstOrDefaultAsync(e => e.Id == id);
            if (app == null)
            {
                return NotFound();
            }
        ...
        [HttpGet]
        public async Task<IActionResult> Front(Guid Id)
        {
            var appoinment = await _context.appoinment.FirstOrDefaultAsync(e => e.Id == Id);
            if (appoinment == null)
            {
            ...
            ViewBag.paths = appoinment.Photo;
            return View();
        }
        [HttpGet]
        public async Task<IActionResult> Rear(Guid Id)
        {
            var appoinment = await _context.appoinment.FirstOrDefaultAsync(e => e.Id == Id);
            if (appoinment == null)
            {
        ...
        [HttpGet]
        public async Task<IActionResult> AttendTest(string test_name, int notc, int minutes, bool isnotanswered)
        {
            var curUser = await _userManager.GetUserAsync(User);
            if (curUser is null)
            {
            ...
            if(notc > 0)
            {
                model.Questions = model.Questions.Take(notc).ToList();
            }
            if(minutes > 0)
            {
                ViewBag.time = minutes;
            }
            if(!!isnotanswered)
            {
                //
