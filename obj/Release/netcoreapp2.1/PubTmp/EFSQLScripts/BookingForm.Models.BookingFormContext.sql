...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624035445_add catalog')
BEGIN
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20190624035445_add catalog', N'2.1.8-servicing-32085');
END;

GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624041536_change catalog')
BEGIN
    DECLARE @var35 sysname;
    SELECT @var35 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Catalog]') AND [c].[name] = N'Cmnd');
    IF @var35 IS NOT NULL EXEC(N'ALTER TABLE [Catalog] DROP CONSTRAINT [' + @var35 + '];');
    ALTER TABLE [Catalog] DROP COLUMN [Cmnd];
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Catalog]') AND [c].[name] = N'Customer');
    IF @var36 IS NOT NULL EXEC(N'ALTER TABLE [Catalog] DROP CONSTRAINT [' + @var36 + '];');
    ALTER TABLE [Catalog] DROP COLUMN [Customer];
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624041536_change catalog')
BEGIN
    EXEC sp_rename N'[Catalog].[PhoneNumber]', N'Name', N'COLUMN';
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624041536_change catalog')
BEGIN
    EXEC sp_rename N'[Catalog].[Acreage]', N'Area', N'COLUMN';
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624041536_change catalog')
BEGIN
    ALTER TABLE [Catalog] ADD [NOBedroom] int NOT NULL DEFAULT 0;
END;
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624041536_change catalog')
BEGIN
    ALTER TABLE [Catalog] ADD [NOWC] int NOT NULL DEFAULT 0;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20190624041536_change catalog', N'2.1.8-servicing-32085');
END;

GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190624043652_chang all catalog''s attr to string')
BEGIN
    DECLARE @var37 sysname;
    SELECT @var37 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Catalog]') AND [c].[name] = N'NOWC');
    IF @var37 IS NOT NULL EXEC(N'ALTER TABLE [Catalog] DROP CONSTRAINT [' + @var37 + '];');
    ALTER TABLE [Catalog] ALTER COLUMN [NOWC] nvarchar(max) NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Catalog]') AND [c].[name] = N'NOBedroom');
    IF @var38 IS NOT NULL EXEC(N'ALTER TABLE [Catalog] DROP CONSTRAINT [' + @var38 + '];');
    ALTER TABLE [Catalog] ALTER COLUMN [NOBedroom] nvarchar(max) NULL;
END;
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Catalog]') AND [c].[name] = N'Area');
    IF @var39 IS NOT NULL EXEC(N'ALTER TABLE [Catalog] DROP CONSTRAINT [' + @var39 + '];');
    ALTER TABLE [Catalog] ALTER COLUMN [Area] nvarchar(max) NULL;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20190624043652_chang all catalog''s attr to string', N'2.1.8-servicing-32085');
END;
